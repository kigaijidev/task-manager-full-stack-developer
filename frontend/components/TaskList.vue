<template>
  ...
  <div class="task-list" v-else>
    <div class="custom-container" v-for="task in viewTodos">
      <div class="checkbox" @click="updateStatusTodo(task.TodoID)">
        <Icon name="typcn:tick" v-if="task.IsCompleted" />
      </div>
      <div class="task" :class="[task.IsCompleted ? 'checked' : '']">
        {{ task.Title }}
      </div>
      <Icon
...
const updateStatusTodo = async (todoId: any) => {
  store.toggleFinished(todoId).catch((err) => {
    apiError.value.msgResponse = err;
  })
}

const deleteTodo = async (todoId: any) => {
  store.deleteTask(todoId).catch((err) => {
    apiError.value.msgResponse = err;
  })
}
</script>
